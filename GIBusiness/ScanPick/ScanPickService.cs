...
{
    public class ScanPickService
    {
        private GIDbContext db;
        private MasterDbContext dbMaster;

        public ScanPickService()
        {
            db = new GIDbContext();
            dbMaster = new MasterDbContext();
        }
        public ScanPickService(GIDbContext db, MasterDbContext dbMaster)
        {
            this.db = db;
        ...
        public actionResultScanPicksearchViewModel FilterTask(ScanPicksearchViewModel model)
        {
            try
            {
                ...
                var query = db.View_Task.Where(c => c.Document_Status == 1 && c.Document_StatusScanPick == null ).AsQueryable();
        ...
        public actionResultScanPicksearchViewModel FilterTaskLabeling(ScanPicksearchViewModel model)
        {
            try
            {
                ...
                //var query = db.IM_Task.Where(c => c.Document_Status == 1 && c.UserAssign == model.userName).AsQueryable();
                //var query = db.IM_Task.Where(c => c.Document_Status == 1).AsQueryable();
                var query = db.View_Task.Where(c => c.Document_Status == 2 && c.Document_StatusScanPick == 2 && c.Document_StatusLabeling == null);
        ...
        public actionResultScanPicksearchViewModel FilterTaskPickQty(ScanPicksearchViewModel model)
        {
            try
            {
                ...
                var query = db.View_Task.Where(c => c.Document_Status == 2 && c.Document_StatusScanPick == 2 && c.Document_StatusLabeling == 2 && c.Document_StatusPickQty == null );
        ...
        public actionResultScanPicksearchViewModel FilterTaskMoveToSelective(ScanPicksearchViewModel model)
        {
            try
            {
                ...
                var query = db.View_Task_moveSTG.Where(c => c.Document_Status == 2 && c.Document_StatusScanPick == 2 && c.Document_StatusLabeling == 2 && c.Document_StatusPickQty == 2 && c.Document_StatusMovetoStgOG == null);
            ...
            try
            {
                ...
                if (ScanPick == RemainScanQty && RemainScanQty > 0)
                {
                    foreach (var TaskItem in ListTaskItem)
                    {
                        ...
                        TaskItem.flag_Filmcuting = 1;
                        TaskItem.flag_Filmcuting_by = model.userName;
                        TaskItem.flag_Filmcuting_Date = DateTime.Now;
        ...
        #region
        public bool task_Filmcutting(ScanPicksearchViewModel model)
        {
            logtxt log = new logtxt();
            String State = "Start";
            String msglog = "";
